...
```{r}
# Hent ut de to delene av datasettet
df1_part1 <- df1[[1]]  # Første del av dataene
df1_part2 <- df1[[2]]  # Andre del av dataene

# Gi like kolonnenavn mellom datasettene for sammenslåing
df1_part2 <- df1_part2 %>%
  rename(år = Tid, statistikkvariabel = ContentsCode)  # Harmoniser kolonnenavnene

# Slå sammen datasettene ved å bruke bind_rows()
df_combined <- bind_rows(df1_part1, df1_part2)

# Rydd opp: Konverter årstall til integer og gi anstendige variabelnavn
df_clean <- df_combined %>%
  as_tibble() %>%  # Konverter til tibble
  mutate(tid = as.integer(år)) %>% 
  select(statistikkvariabel, tid, value) %>%  # Velg de nødvendige kolonnene
  rename(var = statistikkvariabel, verdi = value)  # Gi variablene mer intuitive navn

# Sjekk det ryddede datasettet
df_clean
...
# Filtrer dataene for BNP i både løpende og faste priser, mellom 2000 og 2023
df_bnp <- df_clean %>%
  filter(var %in% c("BNP", "MEMOBNP"), tid >= 2000 & tid <= 2023) %>%
  mutate(verdi = verdi / 1000)  # Konverter verdiene til tusener av kroner

# Lag figuren
ggplot(df_bnp, aes(x = tid, y = verdi, color = var)) +
  geom_line(size = 1.2) +
  labs(title = "BNP per person i tusener av kroner (2000-2023)",
       x = "År",
       y = "BNP per person (tusen kroner)",
       color = "Type BNP") +
  scale_color_manual(values = c("BNP" = "blue", "MEMOBNP" = "red"),
                     labels = c("Løpende priser", "Faste priser")) +
  theme_minimal() +
...
Figuren viser årstallene på X-aksen og BNP på Y-aksen. Vi har to linjer i ploten som viser type BNP. Rød viser utviklingen av Faste priser, mens blå viser Løpende priser. Man kan se at Løpende priser har en eksponentiell utvikling over årene og spesielt fra 2020 og ut, imens Faste priser har hatt en stabil utvikling som knapt har økt i forhold. Årsaken til dette er fordi Løpende priser viser nominell økonomisk vekst, imens Faste priser tar hensyn til inflasjon og endring i pengeverdi.
...
Koden endret datasettet slik at det er færre rader men flere kolonner, årstallene blir heller ikke gjentatt i dette formatet, som gjør det mer oversiktlig.
...
# Beregn relativ endring for BNP og MEMOBNP
df_wide <- df_wide %>%
  mutate(DBNP = 100 * (BNP - LBNP) / LBNP,  # Prosentvis endring i BNP
         DMEMOBNP = 100 * (MEMOBNP - LMEMOBNP) / LMEMOBNP)  # Prosentvis endring i MEMOBNP
...
```{r}
# Besvar oppgave IIc
# Omorganiser datasettet tilbake til langt format med pivot_longer()
df_long <- df_wide %>%
  pivot_longer(cols = c(BNP, MEMOBNP, DBNP, DMEMOBNP),  # Spesifiser kolonnene som skal omorganiseres
               names_to = "var",  # Definer navnet på variabelkolonnen
               values_to = "verdi")  # Definer navnet på verdikolonnen
...
# Filtrer data for vekst i BNP (DBNP) og MEMOBNP (DMEMOBNP) fra 1970 til 2023
df_growth <- df_long %>%
  filter(var %in% c("DBNP", "DMEMOBNP"), tid >= 1970 & tid <= 2023)

# Lag figuren
ggplot(df_growth, aes(x = tid, y = verdi, color = var)) +
  geom_point() +  # Legger til punkter for veksten
  geom_smooth(se = FALSE, method = "loess") +  # Glatter ut linjen for langsiktig trend
  labs(title = "Prosentvis vekst i nominelt og reelt BNP per person (1970-2023)",
       x = "År",
       y = "Prosentvis vekst",
       color = "Type BNP") +
  scale_color_manual(values = c("DBNP" = "blue", "DMEMOBNP" = "red"),
                     labels = c("Nominell BNP", "Reell BNP")) +
  theme_minimal() +
  theme(text = element_text(size = 14),
        plot.title = element_text(hjust = 0.5))
...
Det finnes flere observasjoner av negativ reell BNP per innbygger. Årsakene kan variere men de to nyligste eksemplene som man kan se på ploten er for eksempel i 2008 og 2020. Finanskrisen i 2008 og Covid-19-pandemien i 2020 bisdro til økonomisk nedgang som man ser tydelig utslag av på punktene på grafen.
